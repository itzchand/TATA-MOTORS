...
const Vf1_powersteering= () => {
  const [formData, setFormData] = useState({
    ...
    const formKey = "vehicle factory1-POWER STEERING OIL FM";
    ...
    // Helper function to format label text (e.g., PS_VIN -> Ps Vin)
    ...
    return (
        <div style={formStyle}>
            <h2 style={{ textAlign: "center" }}>VF1 - Power Steering Filling Form</h2>
            <form onSubmit={handleSubmit}>
                {Object.entries(formData).map(([key, value]) => (
                    <div key={key} style={fieldStyle}>
                        ...
                        <input
                            type={key.includes("QTY") || key.includes("PRESSURE") || key.includes("TIME") ? "number" : "text"}
                            id={key}
                            name={key}
                            value={value}
...
export default Vf1_powersteering;